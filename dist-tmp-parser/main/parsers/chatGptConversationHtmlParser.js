...
exports.parseChatGptStandaloneHtml = exports.parseChatGptConversationDocumentHtml = exports.parseChatGptConversationHtmlSnapshot = void 0;
...
const LANGUAGE_ONLY_CODE_PATTERN = /^(mermaid|java|javascript|typescript|python|json|yaml|xml|html|sql|bash|shell|tsx|jsx|css|markdown)$/i;
...
const normalizeText = (value) => decodeHtml(value)
    ...
    .replace(/\n[ \t]+/g, '\n')
    .trim();
const extractStructuredText = (value) => decodeHtml(value)
    .replace(/\u200b/g, '')
    .replace(/<br\s*\/?>/gi, '\n')
    ...
    .replace(/[ \t]+\n/g, '\n')
    .trim();
const isLanguageOnlyCodeText = (value) => LANGUAGE_ONLY_CODE_PATTERN.test(normalizeText(value));
const looksLikeMermaidSource = (value) => {
    const normalizedValue = value.trim();
    if (!normalizedValue || MERMAID_LOADING_TEXT_PATTERN.test(normalizedValue)) {
        return false;
    }
    return (/(^|\n)\s*(flowchart|graph|sequenceDiagram|classDiagram|stateDiagram|erDiagram|journey|gantt|pie|gitGraph|mindmap|timeline|quadrantChart|xychart-beta|xychart|C4Context|C4Container|C4Component|C4Dynamic|C4Deployment|requirementDiagram|block-beta|architecture-beta)\b/.test(normalizedValue) ||
        normalizedValue.includes('-->') ||
        normalizedValue.includes('subgraph'));
};
const looksLikeDiagramSvg = (value) => {
    ...
    const hasSpriteUseOnly = /<use\b/i.test(content) &&
        !/(<path\b|<rect\b|<circle\b|<ellipse\b|<polygon\b|<polyline\b|<line\b|<text\b|<foreignObject\b)/i.test(content);
    const shapeCount = (content.match(/<(path|rect|circle|ellipse|polygon|polyline|line|text|foreignObject)\b/gi)
        ?.length ?? 0);
    if (hasSpriteUseOnly) {
        return false;
    }
...
const extractCodeText = (value) => {
    ...
    return normalizeText(decodeHtml(codeInnerValue)
        .replace(/<br\s*\/?>/gi, '\n')
        .replace(/<\/(div|p|li|blockquote|section|article)>/gi, '\n')
        .replace(/<[^>]+>/g, ''));
};
const extractFallbackCodeText = (value, language) => {
...
const cleanHtmlForMarkdown = (value) => value
...
const inferCodeLanguage = (value) => {
    const directMatch = value.match(/data-gptviewer-code-language="([\w#+.-]+)"/i) ??
    ...
    const labelBeforePreMatch = value.match(/>(mermaid|java|javascript|typescript|python|json|yaml|xml|html|sql|bash|shell|tsx|jsx|css|markdown)</i);
...
const extractCodeBlocks = (value) => {
    const codeBlocks = [];
    const htmlWithoutCodeBlocks = value.replace(/<pre\b[^>]*>([\s\S]*?)<\/pre>/gi, (match, innerHtml) => {
...
const convertHtmlToMarkdown = (value) => {
    ...
    codeBlocks.forEach((codeBlock, index) => {
        const tokenPattern = new RegExp(`(?:${CODE_BLOCK_TOKEN_PREFIX}|GPTVIEWER\\\\?_CODE\\\\?_BLOCK\\\\?_?)${index}(?:${CODE_BLOCK_TOKEN_SUFFIX})?`, 'g');
...
const extractTitleFromHtml = (value) => {
...
const normalizeTitle = (value) => value.replace(TITLE_SUFFIX_PATTERN, '').trim() || 'ChatGPT 대화';
const getSnapshotBlocks = (snapshot) => snapshot.blocks.filter((block) => !!block.html.trim());
const inferRoleFromHtml = (value, index) => {
    const explicitRoleMatch = value.match(/data-message-author-role="(assistant|user)"/i);
...
const extractConversationHtmlContainer = (value) => value.match(/<main\b[\s\S]*?<\/main>/i)?.[0] ??
    value.match(/<body\b[\s\S]*?<\/body>/i)?.[0] ??
    value;
const extractConversationBlocksFromDocumentHtml = (value) => {
    const containerHtml = extractConversationHtmlContainer(value);
    const articleBlocks = [...containerHtml.matchAll(/<article\b[\s\S]*?<\/article>/gi)].map((match) => match[0]);
...
const parseChatGptConversationHtmlSnapshot = (snapshot, fallbackUrl) => {
    ...
    const summarySource = messages.find((message) => message.role === 'assistant')?.text ??
        messages[0]?.text ??
        '';
...
exports.parseChatGptConversationHtmlSnapshot = parseChatGptConversationHtmlSnapshot;
const parseChatGptConversationDocumentHtml = (documentHtml, fallbackUrl) => {
    ...
    return (0, exports.parseChatGptConversationHtmlSnapshot)({
        blocks,
        conversationHtml,
...
exports.parseChatGptConversationDocumentHtml = parseChatGptConversationDocumentHtml;
const parseChatGptStandaloneHtml = (documentHtml, fallbackUrl) => {
    const parsedConversation = (0, exports.parseChatGptConversationDocumentHtml)(documentHtml, fallbackUrl);
...
exports.parseChatGptStandaloneHtml = parseChatGptStandaloneHtml;
